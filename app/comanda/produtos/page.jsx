...
import { buscarComboConfig } from "@/lib/comanda/comboConfig";
import { criarClienteServidor } from "@/lib/supabase/server";

export default async function ProdutosPage() {
  const supabase = await criarClienteServidor();

  const [{ data: produtos }, { data: categorias }, { data: insumos }, categoriasAdicionais, comboConfig] =
    await Promise.all([
      supabase
        .from("produtos")
  ...
  return (
    <section className="w-full max-w-6xl mx-auto px-6 py-10 flex-1">
      ...
      <PainelProdutos
        ...
        comboConfigInicial={comboConfig}
        insumos={insumos ?? []}
      />
    </section>
  );
