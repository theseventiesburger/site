-- Tamanhos por produto: fritas em P/M/G, frango em P/G, e produtos sem
-- tamanho continuam vendendo pelo preço base normalmente. Cada tamanho tem
-- preço próprio e independente (não é acréscimo sobre o preço base) — mais
-- claro pro caixa e pro cliente na hora de escolher. Rode este arquivo
...
create index produto_tamanhos_produto_id_idx on produto_tamanhos (produto_id);

alter table produto_tamanhos enable row level security;

create policy produto_tamanhos_select_publico on produto_tamanhos for select to anon using (true);
create policy produto_tamanhos_select on produto_tamanhos for select to authenticated using (true);
create policy produto_tamanhos_insert_staff on produto_tamanhos for insert to authenticated with check (eh_staff());
create policy produto_tamanhos_update_staff on produto_tamanhos for update to authenticated using (eh_staff()) with check (eh_staff());
create policy produto_tamanhos_delete_staff on produto_tamanhos for delete to authenticated using (eh_staff());

-- criar_pedido: item pode vir com "tamanho_id" — quando vier, o preço do
-- item passa a ser o do tamanho (não soma com o preço base do produto) e o
-- nome exibido ganha o tamanho junto, ex: "Fritas (G)", pra cozinha e
-- relatórios já mostrarem sem precisar de coluna nova. Sem tamanho, produto
-- vende como sempre pelo preço base. Mesma assinatura de antes (0035) — só
-- muda o corpo.
...
begin
  ...
  loop
    ...
    select produtos.vai_para_cozinha, produtos.pode_virar_combo, produtos.preco_combo, produtos.nome, produtos.preco
      into v_vai_cozinha, v_pode_combo, v_preco_combo, v_produto_nome, v_preco_base
    ...
    insert into itens_pedido (
      ...
      v_produto_id,
      v_nome_exibicao,
      coalesce(v_preco_tamanho, v_preco_base) + case when v_eh_combo then v_preco_combo else 0 end,
      (v_item->>'quantidade')::smallint,
      v_item->>'observacao',
      v_item->>'ponto_carne',
      (coalesce(v_preco_tamanho, v_preco_base) + v_soma_adicionais + case when v_eh_combo then v_preco_combo else 0 end)
        * (v_item->>'quantidade')::smallint,
