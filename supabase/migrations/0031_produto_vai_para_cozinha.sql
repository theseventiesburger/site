-- Nem todo item precisa passar pela Cozinha antes de ir pra mesa: bebida
-- (cerveja, refrigerante) é só pegar e servir, sem preparo — só lanche,
-- milk shake, petisco etc. realmente saem da cozinha. Esse controle agora
-- é por produto, marcado no cadastro (Produtos), pra não precisar decidir
-- isso toda vez que lança um pedido. Rode este arquivo inteiro no SQL
-- Editor do Supabase.

alter table produtos add column vai_para_cozinha boolean not null default true;

-- criar_pedido: pra mesa, item com vai_para_cozinha = false não abre ficha
-- na Cozinha — vai direto pra conta já como "entregue". Uma rodada com os
-- dois tipos de item (ex: hambúrguer + cerveja) acaba virando duas fichas
-- (uma de cozinha, outra "direta"), mas continuam na mesma comanda.
-- Delivery/PDV/retirada continuam sempre numa ficha só — não faz sentido
-- separar o motoboy ou o cliente do balcão em duas entregas por causa da
-- bebida. Mesma assinatura de antes (0030) — "create or replace" substitui
-- de verdade, sem overload fantasma.
