-- Bairros de entrega, cada um com seu valor de frete. Endereço de cliente
-- e de pedido passam a guardar bairro/cidade/estado separados (endereco
-- continua sendo só rua e número).
...
create policy bairros_select on bairros for select to authenticated using (true);
create policy bairros_insert on bairros for insert to authenticated with check (true);
create policy bairros_update on bairros for update to authenticated using (true) with check (true);

alter table clientes add column bairro_id uuid references bairros(id);
alter table clientes add column cidade text;
alter table clientes add column estado text;

-- Snapshot no pedido: se o bairro mudar de nome/frete depois, o pedido já
-- feito não muda retroativamente.
alter table pedidos add column bairro_id uuid references bairros(id);
alter table pedidos add column cidade text;
alter table pedidos add column estado text;

-- criar_pedido: aceita os novos campos de endereço.
