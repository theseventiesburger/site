-- A gratuidade de uma categoria de adicional passa a valer por tipo de
-- pedido (ex: troca de pão grátis no salão, mas cobrada no delivery) em
-- vez de um "tudo ou nada" fixo por categoria.
-- Rode este arquivo inteiro no SQL Editor do Supabase.

alter table categorias_adicionais add column gratuita_tipos text[] not null default '{}';

alter table categorias_adicionais
  add constraint categorias_adicionais_gratuita_tipos_check
  check (gratuita_tipos <@ array['mesa', 'delivery', 'pdv']::text[]);

update categorias_adicionais set gratuita_tipos = array['mesa'] where slug = 'paes';

alter table categorias_adicionais drop column gratuita;

-- criar_pedido: o desconto de categoria agora depende do tipo do pedido
-- (p_tipo) em vez de um flag fixo.
