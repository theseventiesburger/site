-- Adicionais deixam de ser vinculados a produto específico no cadastro —
-- passam a ficar disponíveis pra qualquer item direto na hora da venda.
-- A tabela de vínculo não é mais necessária.

drop table if exists produto_adicionais;

-- criar_pedido: valida o adicional só por estar ativo, sem checar vínculo
-- com o produto (não existe mais essa amarração).
