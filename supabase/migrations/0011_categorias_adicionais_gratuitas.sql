-- Categorias de adicionais podem ser marcadas como "gratuita" — o staff
-- escolhe uma opção pra substituir o padrão do produto (ex: trocar o pão)
-- sem cobrar nada, ao contrário de um adicional normal (que soma preço).
-- Rode este arquivo inteiro no SQL Editor do Supabase.

alter table categorias_adicionais add column gratuita boolean not null default false;

update categorias_adicionais set gratuita = true where slug = 'paes';

-- criar_pedido: ignora o preço do adicional quando a categoria dele é
-- gratuita — tanto na soma do subtotal quanto no snapshot salvo em
...
begin
  ...
  loop
    ...
    loop
      select case when coalesce(categorias_adicionais.gratuita, false) then 0 else adicionais.preco end
    ...
    loop
      ...
      select
        v_item_pedido_id,
        adicionais.nome,
        case when coalesce(categorias_adicionais.gratuita, false) then 0 else adicionais.preco end
      from adicionais
      left join categorias_adicionais on categorias_adicionais.id = adicionais.categoria_id
      where adicionais.id = (v_adicional->>'adicional_id')::uuid
        and adicionais.ativo = true;
