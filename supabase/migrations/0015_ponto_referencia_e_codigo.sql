-- Ponto de referência do endereço (cliente e pedido de delivery) e um
-- código curto e único pra identificar o cliente rapidinho (útil quando
-- tem mais de um cliente com o mesmo nome).
-- Rode este arquivo inteiro no SQL Editor do Supabase.

alter table clientes add column ponto_referencia text;
alter table pedidos add column ponto_referencia text;

create or replace function gerar_codigo_cliente() returns text
language plpgsql as $$
declare
  v_codigo text;
  v_existe boolean;
begin
  loop
    ...
    select exists(select 1 from clientes where codigo = v_codigo) into v_existe;
