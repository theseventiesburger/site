-- A Cozinha quer liberar uma rodada aos poucos: numa mesa com hambúrguer,
-- milk shake e batata, a batata pode ficar pronta bem antes do resto, e a
-- equipe quer poder avisar/liberar ela sem esperar o pedido inteiro. Até
-- aqui o status (recebido/preparando/pronto/entregue) só existia no
-- `pedido` inteiro — agora cada item da rodada tem o seu próprio. O status
-- do pedido continua existindo (é usado em pagamento, na fila de "Pedidos
-- Abertos" etc.) mas passa a ser calculado sozinho como o estágio mais
-- atrasado entre os itens da rodada — só avança de coluna na Cozinha
-- quando TODOS os itens já chegaram lá. Rode este arquivo inteiro no SQL
-- Editor do Supabase.

alter table itens_pedido add column status text not null default 'recebido'
  check (status in ('recebido','preparando','pronto','entregue','cancelado'));

-- Pedidos que já estavam entregues antes dessa migration: os itens deles
-- também já estão, de fato, entregues. Exclui mesa antiga sem comanda_id
-- (de antes da 0030) -- tocar nelas aqui dispararia o gatilho de total e
-- esbarraria na constraint comanda_obrigatoria, que essas linhas antigas
-- não cumprem (e nem precisam, já que nunca mais são reabertas).
...
-- ─── pedidos.status = estágio mais atrasado entre os itens da rodada ──────
create function recalcular_status_pedido() returns trigger
...
create trigger itens_pedido_recalcula_status_pedido
  after insert or update of status or delete on itens_pedido
  for each row execute function recalcular_status_pedido();

-- ─── itens_pedido: faltava policy de update (item nunca mudava depois de
-- criado, só via fechar_comanda/fechar_conta_pedido, que são security
-- definer). Agora a Cozinha atualiza status de item direto do cliente.
create policy itens_pedido_update_staff on itens_pedido for update to authenticated using (eh_staff()) with check (eh_staff());

-- ─── criar_pedido: item nasce com o status certo — "entregue" pra item
-- direto (vai_para_cozinha = false), "recebido" (padrão da coluna) pro
-- resto. Mesma assinatura de antes (0031) — só muda o corpo.
