-- Segundo hambúrguer: opção de acrescentar uma segunda carne (mesma
-- proteína) dentro do mesmo lanche por um preço reduzido — não é o mesmo
-- que "combo" (que soma fritas+bebida como itens à parte), é só um
-- acréscimo de preço no próprio item, do mesmo jeito que o combo já soma
-- `preco_combo` em cima do preço base. Mesmo padrão de 0035_combo_dinamico
-- (bool + preço em `produtos`, flag por item em `criar_pedido`), só que
-- como coluna independente — um lanche pode ter as duas opções ao mesmo
-- tempo, sem relação entre si. Rode este arquivo inteiro no SQL Editor do
-- Supabase.

alter table produtos add column permite_segundo_hamburguer boolean not null default false;
alter table produtos add column preco_segundo_hamburguer numeric(10,2) not null default 0 check (preco_segundo_hamburguer >= 0);

-- criar_pedido: mesma assinatura de 0036 — só muda o corpo, pra ler o flag
-- "segundo_hamburguer" do item e somar produtos.preco_segundo_hamburguer no
-- preço unitário/subtotal, igual já faz com combo.
