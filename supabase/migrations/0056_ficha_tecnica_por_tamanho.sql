-- Ficha técnica por tamanho: produto com "10un"/"20un" (ou P/M/G)
-- consumia sempre a mesma receita não importa o tamanho vendido, porque
-- receita_itens só conhecia produto_id — nunca soube qual tamanho saiu, e
-- itens_pedido nem guardava isso. Agora:
--   1) itens_pedido ganha tamanho_id (qual tamanho foi vendido de verdade).
--   2) receita_itens ganha produto_tamanho_id opcional — uma linha pode ser
--      "a receita do produto" (padrão, produto_tamanho_id nulo) ou "a
--      receita específica desse tamanho".
--   3) debitar_estoque_item passa a usar a receita do tamanho vendido
--      quando ela existir; senão cai na receita padrão do produto (mesmo
--      comportamento de sempre, produto sem tamanho nem percebe a mudança).
-- Rode este arquivo inteiro no SQL Editor do Supabase.

alter table itens_pedido add column tamanho_id uuid references produto_tamanhos(id) on delete set null;

alter table receita_itens add column produto_tamanho_id uuid references produto_tamanhos(id) on delete cascade;

alter table receita_itens drop constraint receita_um_vinculo;
alter table receita_itens add constraint receita_um_vinculo check (
  (produto_id is not null and adicional_id is null) or
  (produto_id is null and produto_tamanho_id is null and adicional_id is not null)
);

-- Substitui o unique antigo (produto_id, insumo_id): agora um mesmo produto
-- pode ter uma linha "padrão" (produto_tamanho_id nulo) e uma linha por
-- tamanho pro mesmo insumo, sem colidir.
drop index receita_produto_insumo_unq;
create unique index receita_produto_insumo_unq on receita_itens (
  produto_id,
  coalesce(produto_tamanho_id, '00000000-0000-0000-0000-000000000000'::uuid),
  insumo_id
) where produto_id is not null;

-- criar_pedido: mesma assinatura de 0054 — só muda o corpo, pra gravar
-- tamanho_id no item principal (fritas/bebida do combo não têm tamanho).
...
declare
  ...
  v_eh_combo             boolean;
  v_permite_2_hamburguer boolean;
  v_preco_2_hamburguer   numeric;
  v_eh_2_hamburguer      boolean;
  v_tamanho_id           uuid;
  v_preco_tamanho        numeric;
  v_nome_tamanho         text;
...
begin
  ...
  loop
    ...
    v_eh_2_hamburguer := coalesce((v_item->>'segundo_hamburguer')::boolean, false);
    v_tamanho_id := nullif(v_item->>'tamanho_id', '')::uuid;
    v_combo_bebida_escolhida := nullif(v_item->>'combo_bebida_id', '')::uuid;

    select produtos.vai_para_cozinha, produtos.pode_virar_combo, produtos.preco_combo,
           produtos.permite_segundo_hamburguer, produtos.preco_segundo_hamburguer,
           produtos.nome, produtos.preco
      into v_vai_cozinha, v_pode_combo, v_preco_combo,
           v_permite_2_hamburguer, v_preco_2_hamburguer,
           v_produto_nome, v_preco_base
    ...
    v_preco_tamanho := null;
    v_nome_tamanho := null;
    if v_tamanho_id is not null then
      select preco, nome into v_preco_tamanho, v_nome_tamanho
      from produto_tamanhos where id = v_tamanho_id and produto_id = v_produto_id;

      if not found then
        raise exception 'Tamanho inválido para "%".', v_produto_nome;
      end if;
    end if;

    v_nome_exibicao := case when v_nome_tamanho is not null then v_produto_nome || ' (' || v_nome_tamanho || ')' else v_produto_nome end;
    ...
    if v_eh_combo and v_combo_bebida_escolhida is not null and v_combo_bebida_escolhida <> v_combo_bebida_id then
      if not exists (
        select 1 from produtos escolhida
        join produtos padrao on padrao.id = v_combo_bebida_id
        where escolhida.id = v_combo_bebida_escolhida
          and escolhida.ativo = true
          and escolhida.categoria_id = padrao.categoria_id
      ) then
        raise exception 'Bebida inválida para o combo.';
      end if;
    end if;
    if v_eh_2_hamburguer and not v_permite_2_hamburguer then
      raise exception 'Esse produto não pode receber um segundo hambúrguer.';
    end if;
    ...
    insert into itens_pedido (
      ...
      observacao, ponto_carne, subtotal, status, tamanho_id
    ) values (
      v_pedido_id,
      v_produto_id,
      v_nome_exibicao,
      coalesce(v_preco_tamanho, v_preco_base)
        + case when v_eh_combo then v_preco_combo else 0 end
        + case when v_eh_2_hamburguer then v_preco_2_hamburguer else 0 end,
      (v_item->>'quantidade')::smallint,
      v_item->>'observacao',
      v_item->>'ponto_carne',
      (coalesce(v_preco_tamanho, v_preco_base) + v_soma_adicionais
        + case when v_eh_combo then v_preco_combo else 0 end
        + case when v_eh_2_hamburguer then v_preco_2_hamburguer else 0 end)
        * (v_item->>'quantidade')::smallint,
    ...
    if v_eh_combo then
      foreach v_extra_produto_id in array array[v_combo_fritas_id, coalesce(v_combo_bebida_escolhida, v_combo_bebida_id)]
      loop
...
-- debitar_estoque_item: usa a ficha técnica específica do tamanho vendido
-- quando ela existir pra esse produto+tamanho; senão cai na ficha "padrão"
-- do produto (produto_tamanho_id nulo) — mesmo comportamento de sempre pra
-- quem nunca cadastrou receita por tamanho.
create or replace function debitar_estoque_item() returns trigger
language plpgsql security definer set search_path = public as $$
declare
  r record;
  v_email text;
  v_tem_receita_tamanho boolean;
begin
  select email into v_email from auth.users where id = auth.uid();

  v_tem_receita_tamanho := new.tamanho_id is not null and exists (
    select 1 from receita_itens
    where produto_id = new.produto_id and produto_tamanho_id = new.tamanho_id
  );
  ...
  for r in
    ...
    where produto_id = new.produto_id
      and produto_tamanho_id is not distinct from (
        case when v_tem_receita_tamanho then new.tamanho_id else null end
      )
  loop
