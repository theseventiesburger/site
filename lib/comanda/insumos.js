...
// tipo: 'produto' | 'adicional'. produtoTamanhoId (só faz sentido com
// tipo 'produto'): null/undefined = ficha "padrão" do produto; um id =
// ficha específica daquele tamanho (ver migration 0056 — produto com
// tamanho pode ter as duas, a baixa de estoque usa a do tamanho vendido
// quando ela existir).
export async function listarReceita(supabase, tipo, itemId, produtoTamanhoId = null) {
  ...
  query = tipo === 'produto'
    ? (produtoTamanhoId ? query.eq('produto_tamanho_id', produtoTamanhoId) : query.is('produto_tamanho_id', null))
    : query;
...
export async function adicionarItemReceita(supabase, tipo, itemId, { insumoId, quantidade }, produtoTamanhoId = null) {
  const coluna = tipo === 'produto' ? 'produto_id' : 'adicional_id';
  const { error } = await supabase.from('receita_itens').insert({
    [coluna]: itemId,
    produto_tamanho_id: tipo === 'produto' ? produtoTamanhoId : null,
    insumo_id: insumoId,
    quantidade,
  });
