...
// Todas as comandas abertas, com rodadas e itens — usado em Pedidos
// Abertos, que junta mesa com delivery/pdv/retirada numa lista só.
export async function listarComandasAbertas(supabase) {
...
// Mesma forma de buscarComandaAbertaPorMesa, só que por id — usado na tela
// de Pedidos Abertos, que lista comandas de todas as mesas de uma vez (não
// dá pra filtrar por mesa lá).
export async function buscarComandaPorId(supabase, comandaId) {
  const { data, error } = await supabase
    .from('comandas')
    .select('*, pedidos(*, itens_pedido(*, itens_pedido_adicionais(*)))')
    .eq('id', comandaId)
...
export async function fecharComanda(
  supabase,
  comandaId,
  { formaPagamento, taxaServico, desconto, itensCortesiaIds, itensPrecos }
) {
...
// Cancela a comanda inteira sem passar por pagamento — mesa vazia (nada
// lançado, ou tudo excluído) ou lançamento errado na mesa errada. Cancela
// junto todos os itens ainda ativos das rodadas (ver migration 0044).
export async function cancelarComanda(supabase, comandaId) {
