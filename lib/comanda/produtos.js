...
export async function criarProduto(supabase, dados) {
  const { data, error } = await supabase
    ...
    .insert({
      ...
      tem_ponto_carne: dados.temPontoCarne ?? true,
      pode_virar_combo: dados.podeVirarCombo ?? false,
      preco_combo: dados.podeVirarCombo ? dados.precoCombo || 0 : 0,
      permite_segundo_hamburguer: dados.permiteSegundoHamburguer ?? false,
      preco_segundo_hamburguer: dados.permiteSegundoHamburguer ? dados.precoSegundoHamburguer || 0 : 0,
      nfce_ncm: dados.nfceNcm ?? null,
      nfce_cfop: dados.nfceCfop ?? null,
      nfce_csosn: dados.nfceCsosn || '102',
      ordem: dados.ordem ?? 0,
    })
    .select('id')
    .single();
...
// Substitui de vez os tamanhos do produto (ex: fritas em P/M/G) — apaga os
// antigos e insere os novos numa tacada só, na ordem em que aparecem no
// cadastro. Produto sem tamanho nenhum continua vendendo pelo preço base
// normalmente.
export async function definirTamanhosProduto(supabase, produtoId, tamanhos) {
  const { error: erroExcluir } = await supabase
  ...
  const { error: erroInserir } = await supabase.from('produto_tamanhos').insert(
    tamanhos.map((tamanho, ordem) => ({
