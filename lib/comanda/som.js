...
// Grito curto com um salto rápido de altura (glide sobe) — dá um ar de
// "hee!" animado, sem ser a voz de ninguém, só um oscilador com pitch bend.
function tocarGrito(inicio, duracao, freqInicial, freqFinal) {
  const osc = audioContext.createOscillator();
  const gain = audioContext.createGain();

  osc.type = 'sawtooth';
  osc.frequency.setValueAtTime(freqInicial, inicio);
  osc.frequency.exponentialRampToValueAtTime(freqFinal, inicio + duracao * 0.45);

  gain.gain.setValueAtTime(0, inicio);
  gain.gain.linearRampToValueAtTime(0.5, inicio + 0.015);
  gain.gain.setValueAtTime(0.5, inicio + duracao * 0.5);
  gain.gain.exponentialRampToValueAtTime(0.001, inicio + duracao);
...
// "Hee-hee!" brincalhão — dois gritinhos curtos e animados, diferente do
// sininho de pedido novo, pra dar pra distinguir "hora de olhar o tempo"
// sem nem olhar a tela. Usado no aviso de pedido parado há muito tempo na
// cozinha.
export function tocarAvisoTempo() {
  if (!audioContext || audioContext.state !== 'running') return;

  const agora = audioContext.currentTime;
  tocarGrito(agora, 0.16, 500, 1100);
  tocarGrito(agora + 0.2, 0.2, 550, 1300);
}
