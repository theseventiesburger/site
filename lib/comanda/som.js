...
// Dois bipes graves — diferente do sininho de pedido novo, pra dar pra
// distinguir "hora de olhar o tempo" sem nem olhar a tela. Usado no aviso
// de pedido parado há muito tempo na cozinha.
export function tocarAvisoTempo() {
  if (!audioContext || audioContext.state !== 'running') return;

  const agora = audioContext.currentTime;
  tocarNota(392, agora, 0.22); // sol4
  tocarNota(392, agora + 0.28, 0.22); // sol4
}
