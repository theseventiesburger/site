...
export async function buscarClientes(supabase, termo) {
  ...
  const { data, error } = await supabase
    ...
    .or(`nome.ilike.%${termoLimpo}%,telefone.ilike.%${termoLimpo}%,codigo.ilike.%${termoLimpo}%`)
...
export async function listarClientes(supabase, termo = '') {
  ...
  const termoLimpo = termoDeBusca(termo);
  if (termoLimpo) query = query.or(`nome.ilike.%${termoLimpo}%,telefone.ilike.%${termoLimpo}%,codigo.ilike.%${termoLimpo}%`);
